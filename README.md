...
Remember, to create and send raw packets the root privileges are needed.
...
You can set 1 or more ip addresses domains and file lists included (RHOST/RHOSTS), the number of packets you want to send (COUNT), also the waiting time for a response (TIMEOUT), the number of process (THREADS) and the payload (MESSAGE) too.
After the execution the output has host up or down, latency (min, max, average), packet loss and jitter.
